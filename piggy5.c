			...
			if (strcmp(commands[0],"q") == 0){
				...
				if (right_passive_sock != -1 || right_sock != -1)
					...
					if((right_passive_sock = create_server(ruseport)) != -1){	
						...
						max_fd = (max_fd,right_passive_sock);}
					else
						wAddstr(IO,"An error has occured creating the right connection. Piggy does not have a right side.\n");
				}}
				...
				if (command_count == 1){
					read_fd = open(commands[1],O_RDONLY);
					if (read_fd < 0){
						wAddstr(IO, "Error reading file.\n");
						close(read_fd);
					}else{
						FD_SET(read_fd,&inputs);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/* read from file if one is open and has data to read from */
		if (read_fd != -1 && FD_ISSET(read_fd,&inputs_loop) && stdin_n < BUF_SIZE){
			/* write to stdin_buf but don't clobber data that is there */	
			int read_n = read(read_fd,&stdin_buf[stdin_n],BUF_SIZE - stdin_n);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/* if a null terminated string is in stdbuf then send to left or write buf depending on output settings */
		if (stdin_n != 0 && output_stdin){
			if (outputr && right_sock != -1){
				/* log curses input into prelog */
				if (loglrpre_fd != -1)
					write(loglrpre_fd, stdin_buf, stdin_n);
				Fill_write_buf(right_write_buf, &right_write_n, stdin_buf,stdin_n);
				wAddnstr(OUT_R,stdin_buf,stdin_n);}
			else if (outputl && left_sock != -1){
				if (logrlpre_fd != -1)
					write(logrlpre_fd, stdin_buf, stdin_n);
				Fill_write_buf(left_write_buf, &left_write_n, stdin_buf,stdin_n);
  	...
  	for (j=0;j<n ;j++){
		...
		/* if not a special character then write to screen */
			...
			if (s[j] >= 32 && s[j] <= 126)
				...
				char hex_buf[5];
				snprintf(hex_buf,5,"0x%x",(char)s[j]);
				if ((char)s[j] > 15)
					wAddnstr(i, hex_buf, 4);
				else
					wAddnstr(i, hex_buf, 3);
			}
    		}
	}
  	wrefresh(w[i]);
